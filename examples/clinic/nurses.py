...
class Transfer(Action):
    ...
    You need to complete each step of the tansfer in a conversational manner with each other step by step.
    if you are Wardnurse, your task is to transfer patient to operation room and introduce patient's condition.
    if you are Roomnurse,your task is mainly focus on receive patient and check wardnurseConfirm some key information about the patient.
...
async def debate3(idea: str, investment: float = 3.0, n_round: int = 2):
...
def main(idea: str, investment: float = 3.0, n_round: int =5):
    ...
    asyncio.run(debate3(idea, investment, n_round),)
